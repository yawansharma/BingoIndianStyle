// five_by_five_page.dart
// File: lib/pages/five_by_five_page.dart
...
class _BingoGridState extends State<BingoGrid> {
  List<double> opacities =
      List.generate(25, (index) => 0.0); // Opacity list for animation
  ...
  void didUpdateWidget(BingoGrid oldWidget) {
    ...
    if (!oldWidget.gamePaused && widget.gamePaused) {
      ...
      setState(() {
        opacities = List.generate(25, (index) => 0.0);
      });
    }
    if (oldWidget.gamePaused && !widget.gamePaused) {
...
class _FiveByFiveState extends State<FiveByFive> {
  ...
  final GlobalKey<LogicState> _logicKey = GlobalKey<LogicState>();
  ...
  void _processGameSnapshot(DocumentSnapshot snapshot) {
    ...
    print(
        "Game Data Update: gamePaused = ${gameData['gamePaused']}, currentTurn = ${gameData['currentTurn']}, players = ${gameData['players']}, spectators = ${gameData['spectators']}"); // ADDED LOGGING
  ...
  void _updateBingoState() {
    List<bool> pressedStates = List.generate(25, (index) {
  ...
  void _checkAndShowLeaderboard(Map<String, dynamic> gameData) {
    ...
    // If all players are now spectators, show the leaderboard
    if (players.isEmpty && spectators.isNotEmpty) {
      _showLeaderboardDialog(bingoReactionTimes);
    }
  }
  ...
  void _handleBingoButtonPress() {
    if ((_logicKey.currentState?.completedSets ?? 0) >= 5 &&
        !hasPressedBingo &&
        !isSpectator) {
                ...
                child: Column(
                  children: [
                    SizedBox(
                        height: 0,
                        width: 0,
                        child: Expanded(child: FiveLogic(key: _logicKey))),
                    SafeArea(
                      child: SizedBox(
                        width: 300,
                        child: Row(
                    ...
                    Text(
                      ...
                      style: TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                          color: isSpectator ? Colors.grey : Colors.white),
                    ),
                    const SizedBox(height: 10),
                    const Text('5x5',
